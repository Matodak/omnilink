...
public class UploadEventLogMessageRequest extends UploadMessageRequest {
    public UploadEventLogMessageRequest() {
    }
    
	protected void initialize(SystemTypeEnum model,	ProtocolTypeEnum protocolType) throws CommunicationException {
		ProtocolTypeHelper.ensureOmniLink(protocolType);
		super.initialize(model, protocolType, 0x0D, 0x01);
	}

	protected ReplyMessage createSmallReplyMessage() {
        return new UploadMessageSmallReply(0x0E);
    }

    protected ReplyMessage createReplyMessage() {
        return new UploadEventLogMessageReport();
    }
}
