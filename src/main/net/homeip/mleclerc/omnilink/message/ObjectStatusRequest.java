...
public class ObjectStatusRequest extends RequestMessage {
	private ObjectTypeEnum objectType;
	private int startingObject;
	private int endingObject;

	public ObjectStatusRequest(ObjectTypeEnum objectType, int objectNo) {
		this(objectType, objectNo, objectNo);
	}

	public ObjectStatusRequest(ObjectTypeEnum objectType, int startingObject, int endingObject) {
	...
	protected void initialize(SystemTypeEnum model, ProtocolTypeEnum protocolType) throws CommunicationException {
		...
		super.initialize(model, protocolType, 0x22, 0x06, data);
	}

	protected ReplyMessage createReplyMessage() throws CommunicationException {
		return new ObjectStatusReport(objectType);
	}
}
